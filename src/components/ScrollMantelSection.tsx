...
.wl{position:absolute;inset:0;pointer-events:none;opacity:.12}
...
.wl-dot{width:1px;height:100%;background:#6B4423;box-shadow:0 0 8px rgba(107,68,35,.2)}
...
(function(){
  ...
  function upd(){
    ...
    if(sy===lastY&&window.innerWidth===lastW)return void requestAnimationFrame(upd);
    lastY=sy;lastW=window.innerWidth;
    var r=M.getBoundingClientRect(),h=M.clientHeight,vh=window.innerHeight;
    s=clamp(1-(r.bottom-vh)/(h-vh),0,1);
